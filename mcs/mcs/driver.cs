...
{
    public enum ECompilerAction
    {
        /// <summary>
        /// Continues the Mono compiler as usual after the plugin event returned.
        /// </summary>
        Continue,

        /// <summary>
        /// Leaves the Mono compiler exactly in the state it was in after the
        /// plugin event returned.
        /// </summary>
        Exit,
    }
    ...
    public class CompilerPlugin
    {
        ...
        public CompilerPlugin(TextWriter errorWriter = null)
        {
            _errorWriter = errorWriter;
        }

        protected virtual ECompilerAction AfterEmitButBeforeSaving(ModuleContainer module)
        {
            return ECompilerAction.Continue;
        }

        protected virtual void ChangeSettings(CompilerSettings settings)
        {
        }

        internal ECompilerAction AfterEmitButBeforeSavingAccessor(ModuleContainer module)
        {
            return AfterEmitButBeforeSaving(module);
        }

        internal void ChangeSettingsAccessor(CompilerSettings settings)
        {
        ...
        {
            get { return _errorWriter ?? Console.Out; }
        }
    }
	...
	public class Driver 
	{
		...
		//
		public bool Compile (CompilerPlugin plugin = null)
		{
			var settings = ctx.Settings;

            if (plugin != null) plugin.ChangeSettingsAccessor(settings);
            ...
            if (plugin != null)
            {
                if (plugin.AfterEmitButBeforeSavingAccessor(module) == ECompilerAction.Exit)
                    return true;
            }
	...
	public class CompilerCallableEntryPoint : MarshalByRefObject {
        ...
        public static bool InvokeCompiler (string [] args, CompilerPlugin plugin = null, TextWriter error = null)
        {
            Location.InEmacs = Environment.GetEnvironmentVariable("EMACS") == "t";
            ...
            if (d.Compile(plugin) && d.Report.Errors == 0)
            {
                if (d.Report.Warnings > 0)
                {
                    Console.WriteLine("Compilation succeeded - {0} warning(s)", d.Report.Warnings);
            ...
            //    var d = new Driver(new CompilerContext(setting, new StreamReportPrinter(error ?? plugin.ErrorWriter)));
            //    return d.Compile(plugin);
            //}
            //finally
            //{
            //    Reset();
		...
		{
            return InvokeCompiler(args, null, error);
		}
