...
{
    [Serializable]
    class SCITypeDefinition
    {
        private readonly static Dictionary<object, SCITypeDefinition> s_TypeMappings = new Dictionary<object, SCITypeDefinition>();
        ...
        /// </summary>
        public SCITypeDefinition EnclosingType { get; private set; }
		...
		public bool HasStaticConstructor { get { return Methods.Any(e => e.IsStatic && e.IsConstructor); } }
        ...
        public static SCITypeDefinition Wrap(TypeDefinition def)
        {
            SCITypeDefinition res;

            if (s_TypeMappings.TryGetValue(def, out res))
                return res;

            return new SCITypeDefinition(def);
        }

        public static SCITypeDefinition Wrap(ITypeDefinition def_)
        {
            return Wrap((TypeDefinition)def_);
        }

        public static SCITypeDefinition Wrap(TypeSpec spec)
        {
            SCITypeDefinition res;
            ...
            if (spec.MemberDefinition is ImportedTypeDefinition)
            {
                ...
                res = assembly.NameManager.Context.ResolveImportedType(imported.Namespace + "." + imported.Name);
        ...
        public SCIMemberDefinition ResolveMember(MemberSpec member)
        {
            if(member.Definition is MemberCore)
                return m_MemberMapping[(MemberCore)member.Definition];

            if (member is MethodSpec)
            {
                var imported = SCIMethodDefinition.Wrap(member as MethodSpec);
                return imported;
            }
        ...
        private SCITypeDefinition(TypeDefinition source)
        {
